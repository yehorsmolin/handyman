...
from inventory.filters import InvoiceFilter
...
class InvoiceViewSet(LoginRequiredMixin, viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action in ['list', 'retrieve']:
            return InvoiceReadOnlySerializer
            ...
            try:
                ...
                if vendor_staff.exists():
                    ...
                    if instance.owner.id == vendor_id:
                        ...
                        return Response({'detail': 'User is not associated with any vendor'}, status=403)
            return Response({'detail': 'You do not have permission to access this invoice.'}, status=403)
        else:
            return Response({'detail': 'Authentication credentials were not provided.'}, status=401)
